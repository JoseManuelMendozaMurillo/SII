...
class NuevosAspirantes extends Seeder
{
    public function run()
    {
        ...
        $query->selectMax('id_aspirante');
        $result = $query->get()->getRow();
        $MaxIdAspirante = $result->id_aspirante;
        $MaxIdAspirante = $MaxIdAspirante + 1;

        // Genera varios elementos
        for ($i = $MaxIdAspirante; $i < ($MaxIdAspirante + 200); $i++) {
            $id_aspirante = 0;
            $no_solicitud = $i;
            if (strlen($no_solicitud) < 4) {
                $resta = 4 - strlen($no_solicitud);
                $no_solicitud = str_repeat('0', $resta) . $no_solicitud;
            }
            $nip = '1234';
            ...
            $aspirantes = new UserModelAspirantes();

            $aspirante = new User([
                'name' => $nombre . ' ' . $apellido_paterno . ' ' . $apellido_materno,
                'username' => $curp, // FAKER,
                'email' => $no_solicitud, // FAKER . '@example.com',
                'password' => '1234',
            ]);

            $aspirantes->save($aspirante);
            /*
                        $user = new User([
                            'username' => $curp, // FAKER,
            ...
            $insertID = $aspirantes->getInsertID();
            $aspirante = $aspirantes->findById($insertID);
            $aspirante->addGroup('aspirante');
